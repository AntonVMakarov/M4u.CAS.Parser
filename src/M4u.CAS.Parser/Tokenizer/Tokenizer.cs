    ...
    public TokenizerResult Tokenize(TokenizerRequest request)
    {
        ...
        for (int index = 0; index < request.Expression.Length;)
        {
            ...
            else
            {
                ...
                // во входной строке. Нам нужно выбрать самый длинный результат.
                // Сначала определяем максимальную длину совпадения:
                int maxLength = matches.Max(r => r.parsedLength);

                // Теперь получаем все совпадения с такой (максимальной длиной):
                var bestMatches = matches.Where(r => r.parsedLength == maxLength).ToArray();
                ...
                if (bestMatches.Length != 1)
                {
                    throw new InvalidOperationException(
                        $"Найдено несколько распознавателей с одинаковой максимальной длиной {maxLength}.");
                }

                // У нас единственное совпадение с максимальной длиной:
